...
from tensorrt_llm._utils import str_dtype_to_trt, str_dtype_to_torch
...
'''
def build_rank_model(engine_name, rank, args):
    '''
    ...
    print (f"Rank {rank} model build starts")
...
def build(rank, args):
    ...
    for cur_rank in range(args.world_size):
        ...
        #                           cur_rank, args)
        build_rank_model(engine_name, cur_rank, args)
...
if __name__ == '__main__':
    ...
    if args.parallel_build and args.world_size > 1 and \
        ...
        print ("ERROR: parallel build is not supported yet.")
        sys.exit(1)
        mp.spawn(build, nprocs=args.world_size, args=(args, ))
    else:
        args.parallel_build = False
        #logger.info('Serially build TensorRT engines.')
        logger.info('Serially build TRT-LLM model on ROCm.')
        build(0, args)
